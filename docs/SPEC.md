...
- SSR at `/@slug` (and `/[slug]`), cached in KV, fast TTFB on Cloudflare. CSS-variable theme applied as a `.theme-<name>` class on the page root: base modes `auto`/`light`/`dark` plus v0.2 named presets `mint`/`grape`/`sunset`/`midnight` (each a CSS-variable palette in `globals.css`). Selectable in the editor; the live preview reflects it instantly.
- **Custom theme upload (v1.0):** the `custom` theme stores a hex-only CSS-variable palette in `page.settings.custom_theme` and applies it as an inline `style` on the page root. Hex-only validation (`normalizeCustomTheme`) eliminates any CSS-injection surface; the editor offers color pickers for the nine themeable variables.
...
| **v1.0** ✅ | multi-user + auth (Cloudflare Access; token mode still default); custom theme upload; import/export (portable JSON); custom domains (docs); per-link analytics view | teams; documented per-surface reliability |
